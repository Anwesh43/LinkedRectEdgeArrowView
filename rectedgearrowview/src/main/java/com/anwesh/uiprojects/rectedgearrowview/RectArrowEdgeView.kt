...
class RectArrowEdgeView(ctx : Context) : View(ctx) {
    ...
    data class RAENode(var i : Int, val state : State = State()) {

        private var next : RAENode? = null
        private var prev : RAENode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawRAENode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : RAENode {
            var curr : RAENode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class RectArrowEdge(var i : Int) {

        private var curr : RAENode = RAENode(0)
